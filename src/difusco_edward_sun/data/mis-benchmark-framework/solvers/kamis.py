...
class KaMIS(MWISSolver):
    def __init__(self) -> None:
        ...
        if not kamis_path.exists():
            ...
            subprocess.run(["git", "clone", kamis_repo], cwd=self.directory(), check=False)
            subprocess.run(["git","checkout",target_commit], cwd=kamis_path, check=False)
            subprocess.run(["bash", "compile_withcmake.sh"], cwd=kamis_path, check=False)
    ...
    def solve(self, solve_data_path: pathlib.Path, results_path: pathlib.Path, parameters):
        ...
        res_list = imap_unordered_bar(argumented_solve_graph, solve_data_path.rglob("*.gpickle"), n_processes=3)
...
def solve_graph(graph_path, weighted, directory, cache_directory, results_path, parameters):
    ...
    if not pass_kamis:
        ...
        if weighted:
            discovery = re.compile(r"^(\d+(\.\d*)?) \[(\d+\.\d*)\]$")
            max_mwis_weight = 0
            mis_time = 0.0
            for line in lines:
