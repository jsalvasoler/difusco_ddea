...
class COMetaModel(pl.LightningModule):
    ...
    def categorical_posterior(
        ...
        xt = (
            torch.bernoulli(sum_x_t_target_prob.clamp(0, 1))
            if target_t > 0
            else sum_x_t_target_prob.clamp(min=0)
        )
    ...
    def gaussian_posterior(
        self, target_t: torch.Tensor, t: int, pred: torch.Tensor, xt: torch.Tensor
    ) -> torch.Tensor:
    ...
    def val_dataloader(self):  # noqa: ANN201
        ...
        if len(self.validation_dataset) < self.args.validation_examples:
            error_msg = (
                "Validation dataset is smaller than the number of validation examples."
            )
            raise ValueError(error_msg)
