...
class GaussianDiffusion:
    ...
    def __init__(self, T: int, schedule: str) -> None:
        # Diffusion steps
        self.T = T
    ...
    def __cos_noise(self, t: np.ndarray) -> np.ndarray:
        offset = 0.008
        return np.cos(math.pi * 0.5 * (t / self.T + offset) / (1 + offset)) ** 2

    def sample(self, x0: Tensor, t: int) -> tuple[Tensor, Tensor]:
        # Select noise scales
...
class CategoricalDiffusion:
    ...
    def sample(self, x0_onehot: Tensor, t: int) -> Tensor:
        # Select noise scales
...
class InferenceSchedule:
    ...
    def __call__(self, i: int) -> tuple[int, int]:
        ...
        error_msg = f"Unknown inference schedule: {self.inference_schedule}"
