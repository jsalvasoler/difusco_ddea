...
class HyperbolicRandomGraph(GraphSampler):
    def __init__(self, min_n, max_n, alpha, t, degree, threads):
        ...
        if not girgs_path.exists():
            ...
            subprocess.run(["git", "clone", girgs_repo], cwd=Path(__file__).parent, check=False)
            subprocess.run(["git","checkout", target_commit], cwd=girgs_path, check=False)
            os.mkdir(girgs_path / "build")
            subprocess.run(["cmake", ".."], cwd=girgs_path / "build", check=False)
            subprocess.run(["make", "genhrg"], cwd=girgs_path / "build", check=False)
    ...
    def generate_graph(self):
        ...
        with open(self.tmp_path / "tmp.txt") as file:
