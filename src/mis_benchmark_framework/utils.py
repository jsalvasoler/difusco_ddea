...
def run_command_with_live_output(command, shell=False, capture_output=False):
    if not capture_output:
        result = subprocess.run(command, shell=shell, stdout=sys.stdout, stderr=sys.stderr, text=True, check=False)
        return result, None
    else:
...
def get_available_conda_envs():
    result = subprocess.run(['conda', 'env', 'list'], capture_output=True, text=True, check=False)
