...
@pytest.mark.parametrize("param_type", ["starting_solution", "fix_selection", "fix_unselection", "local_branching", "desired_cost"])
def test_direct_mwis_parameters(param_type: str) -> None:
    ...
    result = maximum_weighted_independent_set(
        adj_matrix,
        weights,
        **params
    )
    ...
    if param_type == "desired_cost" and "desired_cost" in params:
        ...
        assert abs(result.f - params["desired_cost"]) < 1e-6, f"Solution cost {result.f} doesn't match desired cost {params['desired_cost']}"
